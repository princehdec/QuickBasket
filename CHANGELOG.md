...
### Added
- **Pre-order prescription workflow:** added customer-owned prescription submissions tied to a business and specific prescription-required products/quantities, bounded document metadata, pending/approved/rejected/expired state, append-only review events, reviewer endpoints, and transaction-safe order linkage. The additive migration is applied in production; the private Supabase `prescriptions` bucket, authenticated upload endpoint, customer submission/status page, and admin review queue are now implemented. Render still needs the server-only Supabase URL and service-role secret before upload can be enabled in production.
- **Brevo HTTPS OTP testing:** added a server-side Brevo transactional-email adapter that sends bilingual fixed-inbox test OTPs over HTTPS, never exposes the API key to the browser, and returns a generic provider error without logging codes or phone numbers. SMTP remains an inactive fallback because the current Render Free service cannot reliably reach SMTP.
