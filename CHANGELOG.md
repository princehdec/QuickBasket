...
### Added
- **Pre-order prescription workflow:** added customer-owned prescription submissions tied to a business and specific prescription-required products/quantities, bounded document metadata, pending/approved/rejected/expired state, append-only review events, reviewer endpoints, and transaction-safe order linkage. The migration is prepared but not applied; production object-storage upload and customer/admin UI remain before launch.
