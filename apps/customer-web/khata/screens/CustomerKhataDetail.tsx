...
export function CustomerKhataDetail({
  onLogRepayment,
}: {
  onLogRepayment?: () => void;
}) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <KhataHeader title={mockCustomer.name} onBack={() => window.history.back()}>
        <div className="mt-4 flex items-center justify-between rounded-card bg-gradient-to-br from-khata-500 via-khata-600 to-khata-700 p-4 text-khata-50 shadow-[0_6px_18px_-6px_rgb(115_38_29/0.55)]">
          <div>
            ...
            <div className="mt-0.5 flex items-baseline gap-1.5">
              <User size={16} className="self-center text-khata-100" aria-hidden="true" />
              <span className="font-display text-2xl font-extrabold tracking-tight tabular-nums">
                ₹{mockCustomer.khataBalance.toFixed(2)}
              </span>
          ...
          <div className="flex gap-2">
            <button
              type="button"
              className="flex h-9 items-center rounded-button bg-khata-50 px-3.5 font-display text-sm font-bold text-khata-700 shadow-sm transition-all duration-150 hover:bg-white active:scale-95"
            >
              Call
            </button>
            {onLogRepayment && (
              <button
                ...
                className="flex h-9 items-center rounded-button bg-brand-600 px-3.5 font-display text-sm font-bold text-paper-50 shadow-sm transition-all duration-150 hover:bg-brand-700 active:scale-95"
              >
                Log Repayment
              </button>
            )}
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 text-sm tabular-nums">
          <div className="flex items-center gap-2 text-gray-700">
            ...
            <span>{mockCustomer.phoneNumber}</span>
          </div>
          <div className="flex items-center gap-2 text-gray-700">
